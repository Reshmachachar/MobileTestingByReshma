...
import PageObject.EMICalculator;
import Utils.ExtentListeners;
import Utils.XLUtility;

public class EMIcalculatorTest extends BrowserBaseTest2 {
	XLUtility testdata;
	ExtentListeners e=new ExtentListeners();
	//An_ExtentReport extentTest=new An_ExtentReport();
	public static EMICalculator homepage;

	@Test
	public void TC01_ExplorePoduct() throws Exception 
	{
		homepage=new EMICalculator(driver);

		//click on loan
		Thread.sleep(2000);
		homepage.getLoan().click();
		e.test.log(Status.INFO,"Click on Loan is successfull" );
		Thread.sleep(2000);
		//click on Calculate EMI
		homepage.getCalculateEMI().click();
		e.test.log(Status.INFO,"Click on Calculate EMI is successfull" );

		//CLICK ON EMI calculator
		Thread.sleep(2000);
		homepage.getEmiCalculate().click();
		e.test.log(Status.INFO,"Click on EMI Calculator is successfull" );
